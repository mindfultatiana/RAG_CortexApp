...
def config_options():
    ...
    categories = (
        session.table("docs_chunks_table").select("category").distinct().collect()
    )
    ...
    st.sidebar.checkbox(
        "Do you want that I remember the chat history?",
        key="use_chat_history",
        value=True,
    )

    st.sidebar.checkbox(
        "Debug: Click to see summary generated of previous conversation",
        key="debug",
        value=True,
    )
...
def main():
    ...
    st.write(
        "This is the list of documents you already have and that will be used to answer your questions:"
    )
